(() => {
  ...
  const background = Bodies.rectangle(240, 360, 480, 720, {
    isStatic: true,
    render: { fillStyle: "rgb(74, 176, 235)" },
  });
  ...
  function resize() {
    ...
    parent.style.setProperty("--floor-height", `${floor.offsetHeight}px`);
  ...
  function newBall(x, y, size) {
    ...
    const textureSize =
      size === 1
        ? { width: 243, height: 242 }
        : size === 4
          ? { width: 278, height: 254 }
          : { width: 1254, height: 1254 };
    const targetDiameter = 243 * (spriteScale[size] ?? size / 12.75);
    const scale = targetDiameter / textureSize.width;
    const collisionRadius = targetDiameter * (collisionScale[size] ?? 0.45);
    const density = 0.001 + size * 0.0002;

    const textureIndex = size <= 13 ? size : 13;

    c = Bodies.circle(x, y, collisionRadius, {
      density,
      render: {
        sprite: {
          texture: `assets/img/${textureIndex}.png`,
          xScale: scale,
          yScale: targetDiameter / textureSize.height,
        },
      },
    });
    c.size = size;
